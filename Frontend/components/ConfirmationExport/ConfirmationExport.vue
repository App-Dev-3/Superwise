...
const logoSrc = new URL('/public/images/appLanding_logo_light.svg', import.meta.url).href
...
function drawCanvas() {
    ...
    logo.onload = () => {
        ...
        ctx.fillText('Thesis Supervision Confirmation', width / 2, titleY)
        ...
        ctx.fillText('Student:', centerX, y)
        y += gap
        ...
        ctx.font = `bold 36px ${fontFamily}`
        ctx.fillStyle = '#000000'
        ctx.fillText('Supervisor:', centerX, y)
        y += gap
        ...
        ctx.font = `bold 36px ${fontFamily}`
        ctx.fillStyle = '#000000'
        ctx.fillText('Confirmation Details', centerX, y)
        y += gap

        ctx.font = `30px ${fontFamily}`
        ctx.fillStyle = '#000000'
        ctx.fillText(`Accepted on: ${props.acceptedDate}`, centerX, y)
        ...
        if (props.confirmationUrl.length > 0) {
            ...
            qrImage.onload = () => {
                ...
                ctx.fillText('Scan to verify confirmation', centerX, qrY + qrSize + 30)
...
<template>
    <div class="hidden">
        <canvas :id="props.canvasId" ref="canvas" width="1920" height="1080" class=""/>
    </div>
</template>
