...
if (!studentStore.studentProfile) {
    if (!userStore.user) {
        await userStore.refetchCurrentUser()
            .then( () => {
                studentStore.fetchStudentProfile(userStore.user?.id || '')
                    .then(() => {
...
const handleSave = async () => {
    ...
    if (!studentStore.studentProfile) {
        await studentStore.fetchStudentProfile(userStore.user?.id || '');
    }
    ...
    if (topicDescription.value !== studentStore.studentProfile?.thesis_description) {
        const {status} = useFetch('/api/students/' + studentStore.studentProfile?.id, {
            method: HttpMethods.PATCH,
            body: {
