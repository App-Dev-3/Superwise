...
watch(
    ...
    async (email) => {
      ...
      if (!userStore.user) {
        const fetched = (await getUserByEmail(
            email,
        )) as UserData;
      ...
      if (current_user.value?.id) {
        supervisor_data.value =
            (await getSupervisorByUserId(
    ...
    <div class="min-h-screen flex flex-col max-w-7xl w-full">
      ...
      <div>
        <ActionCard
            :button-text="
						t(
							'dashboard.supervisor.manageStudents',
						)
					"
            card-type="primary"
            ...
            @action-button-clicked="
						navigate(
							'/supervisor/currently-supervising',
						)
					"
        >
          <div class="py-8 px-16">
            ...
            <p class="text-md">
              {{
                t(
                    'dashboard.supervisor.slotsFilled',
                )
              }}
            </p>
          </div>
