...
const removeStudent = (studentId) => {
  ...
  const index = mockStudents.value.findIndex(student => student.id === studentId);
  if (index !== -1) {
...
const addStudent = (email) => {
  ...
  const existingStudent = mockStudents.value.find(student => student.email === email);
  if (!existingStudent) {
    ...
    const newStudent = {
      ...
      lastName: "Student",
      email: email,
      src: `https://randomuser.me/api/portraits/${id % 2 === 0 ? 'men' : 'women'}/${mockStudents.value.length + 1}.jpg`,
    };
...
<template>
  ...
  <div class="w-full h-screen">
    <AdminHeader
        header-text="Edit Supervising Students"
    />
